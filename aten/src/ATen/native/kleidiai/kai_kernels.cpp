...
/**
 * @brief We maintain a global buffer for Input that needs quantization and
 * packing This is done to avoid memory allocations for every matmul input. For
 * now there is no case where the the same buffer is shared across parallel
 * inputs
 */
size_t global_buffer_bytes = 11008 * 4096 * 4;
uint8_t* lhs_packed =
    static_cast<uint8_t*>(aligned_alloc(64, global_buffer_bytes));
...
void kai_quant_pack_lhs_int4_mm_groupwise(
  ...
  void* dst_ptr = (void*)((uint8_t*)lhs_packed);

  // Quantize and pack the Input for LLM
  kai_run_lhs_quant_pack_qsi8d32p_f32(
      m, k, bl, mr, kr, sr, 0, src_ptr, sizeof(float) * k, dst_ptr);
...
void kai_quant_pack_lhs_int4_mm_channelwise(
    const Tensor& output,
    const Tensor& input,
    const Tensor& weight,
    const int64_t m,
  ...
  at::parallel_for(
      0, num_threads_y * num_threads_x, 0, [&](int64_t begin, int64_t end) {
